...
function RevealArea( event )
	local status, nextCall = Error_debug.ErrorCheck(function() 
		...
		local units = FindUnitsInRadius(caster:GetTeamNumber(), point , nil, visionRadius , DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_HERO , DOTA_UNIT_TARGET_FLAG_NONE, 0 , false)
		local timeElapsed = 0
	...
	--	unit:SetNightTimeVisionRange(visionRadius)
		Timers:CreateTimer(0.03,function()
			for _,unit in pairs(units) do
				if unit ~= nil then
					if unit:HasModifier("modifier_invisible")  then -- and not unit:HasModifier("modifier_invisible_truesight_immune")
				...
				--		unit:AddNewModifier(unit, nil, "modifier_invisible_truesight_immune", {Duration = 10})
				--	end
